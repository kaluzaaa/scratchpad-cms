...
import { episodesProjection } from "./episodes/readModel";
...
export const getEventStore = (db: D1Database): Promise<SQLiteEventStore> =>
  (initialized ??= (async () => {
    const store = getSQLiteEventStore({
      ...
      projections: projections.inline([episodesProjection]),
    });
    // Also runs inline projections' init (creates the episodes collection).
